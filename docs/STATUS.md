...
- **Branch** `remediation/plan` (104+ commits ahead of `master`, which is untouched at 7002a71).
  Worktrees `../neural_trade_gates` and `../neural_trade_ablation` (detached at 6dec27a) hold the
  gate and ablation runs; leave them.
- **CI is red** since fe4ba85 (the `unit` job). Cause, reproduced locally: CI pins plotly 5.24.1,
  which writes figure arrays as JSON lists, so two figure size-budget tests fail there
  (`test_viz_delta.py::test_no_empty_panel_and_size_budget_on_a_full_size_block`,
...
2. **NT-007, which delta the strategies read.** `magnitude_coherent` and the delta-based
...
3. **NT-006, re-run the physics ablation on the current trainer:** about 7-10 GPU hours.
   Recommendation: yes, after NT-001, on a night the GPU is free.
4. **NT-009, disk:** C: has a few GB free because of the Docker WSL image (~116 GB). Only you can
   free it. `Bitcoin_BTCUSDT.csv` (291 MB, repo root, unreferenced) is also yours to keep or delete.
5. **NT-017, licence:** none chosen.
6. **NT-008, merge into `master`:** when you are ready, after CI is green (this also lets the
...
3. **NT-010** tracked evidence for cited numbers (P2), then **NT-003** direction skill (P1,
   experimenter; GPU).
