...
# -----------------------------
# Refactored Config (addresses P0-1/2/3 + P1-1/2/3/4/5).
# @dataclass for structure + validation + computed derived (P0-3).
# LAMBDA_VAC default now 0 (P0-2). Single VAR_FLOOR (P0-1).
# Legacy kept with notes (P1-1). validate() for coupling (P1-4).
# See math test stubs (priority area).
from dataclasses import dataclass, field
import warnings
import logging
...
# Focal loss hyperparameters for direction classification
    ...
    # --- Methods added for P0/P1 (validate, properties, deprecation notes) ---

    def __post_init__(self):
        # Defensive for any direct setattr users (dataclass handles most via field).
        if isinstance(getattr(self, 'EXTENDED_TREND_PERIODS', None), tuple):
            object.__setattr__(self, 'EXTENDED_TREND_PERIODS', list(self.EXTENDED_TREND_PERIODS))
        if isinstance(getattr(self, 'HORIZON_STEPS', None), tuple):
            object.__setattr__(self, 'HORIZON_STEPS', list(self.HORIZON_STEPS))
        self.validate()

    def validate(self) -> None:
        """Validate key invariants (P0-3 + P1-4). Called from __post_init__ and
        early in DataProcessor / train_and_evaluate.
        """
        # Basic positives / ranges
        if self.LOOKBACK <= 0:
            raise ValueError("LOOKBACK must be positive")
        if self.LOOKBACK > 1440:
            raise ValueError("LOOKBACK should not exceed 1 day for minute data")
        if self.BATCH_SIZE < 16 or self.BATCH_SIZE > 2048:
            raise ValueError("BATCH_SIZE should be between 16 and 2048")
        if self.LR <= 0 or self.LR > 1.0:
            raise ValueError("LR should be >0 and <=1.0")
        if self.EPOCHS < 1:
            raise ValueError("EPOCHS must be >=1")

        # Horizon / trend list coupling (P1-4)
        if not self.HORIZON_STEPS or any(h <= 0 for h in self.HORIZON_STEPS):
            raise ValueError("HORIZON_STEPS must be non-empty list of positive ints")
        if not self.EXTENDED_TREND_PERIODS or any(t <= 0 for t in self.EXTENDED_TREND_PERIODS):
            raise ValueError("EXTENDED_TREND_PERIODS must be non-empty list of positive ints")
        if len(self.HORIZON_STEPS) != len(self.EXTENDED_TREND_PERIODS):
            raise ValueError(
                f"HORIZON_STEPS ({self.HORIZON_STEPS}) length must match "
        ...
        if self.HORIZON_STEPS != sorted(self.HORIZON_STEPS):
            raise ValueError("HORIZON_STEPS should be ascending")
        if self.EXTENDED_TREND_PERIODS != sorted(self.EXTENDED_TREND_PERIODS):
            raise ValueError("EXTENDED_TREND_PERIODS should be ascending")

        # Consolidated VAR (P0-1) — we chose the 1e-4 stabilizing value
        if self.VAR_FLOOR != 1e-4:
            warnings.warn(
                f"Config.VAR_FLOOR={self.VAR_FLOOR} (expected 1e-4 after P0-1 consolidation). "
                "Using provided value.",
                DeprecationWarning,
                stacklevel=2,
            )
        if self.VAR_CAP <= self.VAR_FLOOR:
            raise ValueError("VAR_CAP must be > VAR_FLOOR")

        # LAMBDA_VAC (P0-2) — now 0 by default (opt-in)
        if self.LAMBDA_VAC > 0:
            logging.getLogger(__name__).warning(
                "Config.LAMBDA_VAC > 0 (vacuum_bandwidth_loss active). "
                "Old default was 1.0 (always-on); now opt-in (P0-2 fix)."
            )

    # Computed / derived (P0-3)
    @property
    def momentum_clip_max(self) -> float:
        """Derived from LOOKBACK (replaces class-level MOMENTUM_CLIP_MAX = LOOKBACK)."""
        return float(self.LOOKBACK)
...
# Structured view over the 10 model outputs (9 predictive heads + vacuum_overflow).
# Used to replace magic indexing ([0],[3],[6] etc.) and 9-way unpacks in the
# training/eval paths. The underlying Keras Model still returns a list/tuple of
# tensors (for multi-output compatibility), but call sites now do:
#   heads = PredictiveOutputs(*y_pred_list)
#   ... heads.price_h0 ...
# This makes adding aux outputs or reordering far less error-prone.
