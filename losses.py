...
This module provides a simple `Losses` dataclass-like registry with a
`register` decorator. Decorating any function (or instance method) with
`@Losses.register()` will add it to `Losses.registry` under the function
name (or a provided name). This allows discovery and programmatic lookup
of losses without changing the implementation sites.
...
class Losses:
    ...
    Usage:
        ...
        Losses.get('my_loss')(...)  # returns the same callable
    """

    # Class-level registry mapping name -> callable (ignored by dataclass)
    registry: ClassVar[Dict[str, Callable]] = {}

    @classmethod
    def register(cls, name: str = None):
        """Decorator that registers the function under `name` or its __name__.

        The decorator returns the original function unchanged so it can
        continue to be used as a normal function/method.
        """
        def decorator(func: Callable):
            key = name or func.__name__
            if key in cls.registry:
                warnings.warn(f"Overwriting existing loss registration: {key}")
